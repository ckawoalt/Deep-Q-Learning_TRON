...
class Net(nn.Module):
    ...
    def get_value(self, x,env_prob=None):
        if env_prob is not None:
            value, actor_output = self(x, env_prob)
        else:
            value, actor_output = self(x)
    ...
    def evaluate_actions(self, x, actions,env_prob=None):
