...
class Ai(Player):
    ...
    def __init__(self, epsilon=0):
        ...
        #self.net =ResNet(resnet.BasicBlock, [3, 4, 6, 3], 10, True).to(device)
...
def train(model):
    ...
    # player_2 = Ai(epsilon)
        ...
        pred_q_values_batch = torch.sum(model(old_state_batch.to(device)).gather(1, action_batch.to(device)), dim=1).to(device)
        #pred_q_values_batch=model(old_state_batch.to(device))
       ...
       # Compute targeted Q-value for action performed
        ...
        # target_q_values_batch = torch.cat(tuple(reward_batch if batch[4][i]
        #                                         else reward_batch + model.gamma * torch.max(pred_q_values_next_batch)
        #                                         for i in range(len(reward_batch))))
        ...
        #print(target_q_values_batch,pred_q_values_batch)
        # Compute the loss
...
def main():
    model = Net().to(device)
    #model = ResNet(resnet.BasicBlock, [3, 4, 6, 3], 10, True).to(device)
    if os.path.isfile('ais/' + folderName + '/ai.bak'):
